...
let rec print_coords = function
  | []     -> Printf.printf "\n"
  | hd::tl -> let x, y, step = hd in
              let () = Printf.printf "(%d,%d - %d) " x y step in
              print_coords tl;;

let same_coordinates a b =
  let x1, y1, _ = a in
  let x2, y2, _ = b in
  (x1 = x2) && (y1 = y2);;

let summed_signals a b =
  let x, y, step1 = a in
  let _, _, step2 = b in
  (x, y, step1 + step2);;
  
(* split a string such as "R15" into a direction ("; "R"; ") and a distance (15) *)
let split_direction x =
  let n = String.length x in
  x.[0], int_of_string (String.sub x 1 (n - 1));;

(* Build a list of coordinates from a starting point following a direction and a distance.
   Note that the list is reversed (the starting point is the last element of the built list) *)
let rec advance start direction = function
  | 0 -> [start]
  | n -> let x,y,step = start in
         (match direction with
          | 'U' -> advance (x, y+1, step+1) direction (n-1) @ [start]
		  | 'D' -> advance (x, y-1, step+1) direction (n-1) @ [start]
		  | 'R' -> advance (x+1, y, step+1) direction (n-1) @ [start]
		  | 'L' -> advance (x-1, y, step+1) direction (n-1) @ [start]
          | _   -> failwith "Unkown direction");;

(* Transform a starting point and a path into a list of coordinates for the path.
   The list is built "backwards" for efficiency purposes: building the coordinate list 
   "origin first" would be O(n^2) due to repeated concatenation of the accumulator. *)
let rec coords_from_path acc = function
  | []    -> acc 
  | x::xs -> let direction, distance = split_direction x in
			 let starting_point::rest_of_acc = acc in
             let new_segment = advance starting_point direction distance in
             coords_from_path (new_segment @ rest_of_acc) xs;;

(* filter two lists, keeping only common coordinates, summing signal times *)
let rec common_coordinates acc xx yy = match xx, yy with
  | [], _        -> acc
  | _, []        -> acc
  | x::xs, y::ys -> if same_coordinates x y then
					  common_coordinates ((summed_signals x y)::acc) xs ys
					else if x < y then
					  common_coordinates acc xs (y::ys)
					else
					  common_coordinates acc (x::xs) ys;;

let manhattan_distance (x, y, _) = abs(x) + abs(y);;

let signal_delay (_, _, step) = step;;

let non_zero_minimum l =
  let rec first_non_zero = function
    | [] -> failwith "empty list or no non-zero value"
	| x::xs when x = 0 -> first_non_zero xs
	| x::_ -> x in
  first_non_zero (List.sort compare l);;
  

(* The strategy for both parts is:
    - build the list of coordinates for each path
    - sort each list of coordinates
    - build the list of common coordinates (this is why the lists are sorted)
    - calculate the distance for each point
    - pick the min
   The difference between the two parts is how the distance is calculated. *)
   
let pick_best_crossing weight_function first_path second_path =
  let coords1 = List.sort compare (coords_from_path [origin] first_path) in  
  let coords2 = List.sort compare (coords_from_path [origin] second_path) in
  let crossings = common_coordinates [] coords1 coords2 in
  (* let () = print_coords crossings in *)
  let distances = List.map weight_function crossings in
  non_zero_minimum distances;;

let part1 = pick_best_crossing manhattan_distance;;
let part2 = pick_best_crossing signal_delay;;

